...
import { sendTicketResponseEmail, sendTicketResolvedEmail } from '@/lib/supportTicketEmails';
...
export default function TicketDetailPage() {
  ...
  const handleAddMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Send email to customer about the response
      if (ticket?.user?.email) {
        await sendTicketResponseEmail(ticket.user.email, params.id as string, ticket.subject, newMessage);
      }

      toast.success('Message added and email sent to customer');
      setNewMessage('');
      loadMessages();
    } catch (error: any) {
  ...
  const handleStatusChange = async () => {
    ...
    try {
      ...
      if (newStatus === 'resolved' && ticket?.user?.email) {
        await sendTicketResolvedEmail(ticket.user.email, params.id as string, ticket.subject);
      }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          {/* Messages */}
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Messages</h2>
                ...
                messages.map((msg) => (
                  <div key={msg.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-medium text-gray-900 dark:text-gray-100">{msg.user?.email}</span>
            ...
            {/* Add message form */}
            <form onSubmit={handleAddMessage} className="border-t border-gray-200 dark:border-gray-700 pt-6">
              <textarea
                value={newMessage}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            ...
            <p className="text-lg font-semibold text-gray-900 dark:text-gray-100 capitalize">{ticket.priority}</p>
          </div>

          {/* Customer */}
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-3">Customer</h3>
            <p className="text-gray-900 dark:text-gray-100">{ticket.user?.email}</p>
          </div>

          {/* Category */}
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-3">Category</h3>
            <p className="text-gray-900 dark:text-gray-100">{ticket.category?.name}</p>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            ...
            <div className="space-y-2 text-sm">
              <div>
                ...
                <p className="text-gray-900 dark:text-gray-100">{new Date(ticket.created_at).toLocaleString()}</p>
              </div>
              <div>
                <p className="text-gray-500 dark:text-gray-400">Updated</p>
                <p className="text-gray-900 dark:text-gray-100">{new Date(ticket.updated_at).toLocaleString()}</p>
              </div>
            </div>
          </div>
        </div>
