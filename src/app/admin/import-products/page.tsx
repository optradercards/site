...
export default function CardImportPage() {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      // Build pipeline steps: card batch → history batch, chained sequentially
          ...
          <div>
            ...
            <div className="flex gap-3">
              ...
              <select
                ...
                className="px-4 py-4 bg-gray-50 dark:bg-slate-800/50 border-2 border-gray-300 dark:border-slate-700/50 rounded-xl text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent text-lg"
              >
                ...
                <option value="trendingToday">Trending Today</option>
              </select>
              <button
                type="submit"
      ...
      {searchResults &&
        ...
        searchResults.items.length > 0 && (
          <>
            ...
            {pipelineStatus === 'completed' && (
              <div className="bg-green-50 dark:bg-green-900/30 border border-green-300 dark:border-green-700 rounded-xl p-6">
                ...
                <div className="grid grid-cols-3 gap-4 text-sm">
                  {(() => {
                    const cardJobs = jobs.filter((j) => j.platform === "shiny-cards");
                    const totals = cardJobs.reduce(
                      (acc, j) => ({
                        brands: acc.brands + (j.stats.brands_imported ?? 0),
                        sets: acc.sets + (j.stats.sets_imported ?? 0),
                        products: acc.products + (j.stats.products_imported ?? 0),
                      }),
                      { brands: 0, sets: 0, products: 0 }
                    );
                    return (
                      <>
                        <div>
                          <span className="text-green-700 dark:text-green-200">Brands:</span>{" "}
                          <span className="text-gray-900 dark:text-white font-semibold">{totals.brands}</span>
                        </div>
                        <div>
                          <span className="text-green-700 dark:text-green-200">Sets:</span>{" "}
                          <span className="text-gray-900 dark:text-white font-semibold">{totals.sets}</span>
                        </div>
                        <div>
                          <span className="text-green-700 dark:text-green-200">Products:</span>{" "}
                          <span className="text-gray-900 dark:text-white font-semibold">{totals.products}</span>
                        </div>
                      </>
                    );
                  })()}
            ...
            {pipelineStatus === 'running' && jobs.length > 0 && (
