  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!user) {
        ...
        router.push(`/login?returnUrl=/support/create?subject=${encodeURIComponent(formData.subject)}&message=${encodeURIComponent(formData.message)}`);
        return;
      }
      ...
      await sendTicketCreatedEmail(formData.email, ticketId, formData.subject);

      toast.success('Thank you! Your message has been sent and a support ticket has been created.');
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      {!isLoggedIn && (
        <div className="bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 text-sm text-blue-800 dark:text-blue-200">
          ...
          <p className="mb-3">Sign in to create a support ticket and track responses.</p>
          <a
