...
export function ImportPreview({ selectedBrands, sets, groups, brands }: ImportPreviewProps) {
...
function CountBadge({ label, count, color }: { label: string; count: number; color: 'blue' | 'purple' }) {
...
function ItemCard({
  ...
  const hoverClass = hoverColor === 'blue'
    ? 'hover:border-blue-500/30 group-hover:text-blue-300'
    : 'hover:border-purple-500/30 group-hover:text-purple-300';

  return (
    <div className={`group flex items-center gap-4 p-4 bg-slate-800/40 hover:bg-slate-800/60 rounded-xl border border-slate-700/50 ${hoverClass} transition-all`}>
      ...
      <div className="flex-1 min-w-0">
        <p className={`text-white text-sm font-semibold truncate ${hoverClass} transition-colors`}>
          {name}
        </p>
